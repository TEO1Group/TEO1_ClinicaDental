...
import {UsuarioSistemaRequest, UsuarioSistemaResponse, UsuarioUpdateRequest, EstadoUsuarioRequest} from '../models/usuario-sistema.model';
...
export class UsuarioSistemaService extends ApiService {
  ...
  actualizarUsuario(id: string, request: UsuarioUpdateRequest): Observable<UsuarioListado> {
    return this.http.put<UsuarioListado>(`${this.crearUsuarioUrl}/${id}`, request);
  }

  cambiarEstado(id: string, request: EstadoUsuarioRequest): Observable<UsuarioListado> {
    return this.http.patch<UsuarioListado>(`${this.crearUsuarioUrl}/${id}/estado`, request);
  }
}